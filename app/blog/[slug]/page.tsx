...
import { generateTableOfContents, getPosts, getSinglePost, processHtmlContent } from "@/lib/blog";
...
async function Page({ params }: PageProps) {
	...
	const formattedDate = new Date(data.post.publishedAt).toLocaleDateString("en-US", {
		day: "numeric",
		month: "long",
		year: "numeric",
	});
		...
		<BlogContainer className="pt-10 pb-28">
			<section className="space-y-6 lg:space-y-8">
				...
				<div className="flex flex-col items-center gap-4">
					<time dateTime={data.post.publishedAt.toString()} className="text-muted-foreground">
						{formattedDate}
					</time>
					<h1 className="text-3xl lg:text-4xl font-semibold text-center text-foreground">{data.post.title}</h1>
