...
export function PageWrapper({ children, goBack = false, hasPrivacy = false }: PageWrapperProps) {
	...
	return (
		<PageWrapperContext.Provider value={true}>
			<div className="w-full max-w-7xl flex flex-col gap-4">
				<div className={cn("flex gap-4 relative", goBack ? "justify-between" : "justify-end")}>
					{goBack && (
						<Button
...
export function Container({ children, className, orientation = "vertical", ...props }: Container) {
	...
	return (
		<div
			className={cn("flex gap-4", orientation === "horizontal" ? "flex-row flex-wrap" : "flex-col", className)}
			{...props}
		>
			{children}
		</div>
...
export function Title({ children, size, separator = false, ...props }: TitleProps) {
	const isWithinPageWrapper = useContext(PageWrapperContext);
...
export function Button({ children, className, variant = "default", ...props }: ButtonProps) {
	...
	return (
		<ShadcnButton className={cn("w-fit py-3 px-5 rounded-md", className)} variant={variant} {...props}>
			{children}
		</ShadcnButton>
	);
}
...
export function ImageWrapper({ src, alt, size = "medium", zoom = false }: ImageWrapperProps) {
	const isWithinPageWrapper = useContext(PageWrapperContext);
...
export function List({ children, position = "inside", listStyle = "disc", className, ...props }: ListProps) {
	const isWithinPageWrapper = useContext(PageWrapperContext);
	...
	return (
		<ListContext.Provider value={true}>
			<ul className={cn(`list-${listStyle} list-${position}`, className)} {...props}>
				{children}
			</ul>
...
export function SystemRequirementsWrapper({ requirements }: SystemRequirementsWrapperProps) {
	const isWithinPageWrapper = useContext(PageWrapperContext);
			...
			<div className="flex flex-col gap-4 sm:flex-row sm:gap-12">
				{rm && (
					<div className="flex flex-col max-w-md gap-1.5">
						...
						{rm.additional && (
							<ItemWrapper>
								<SubText>Additional Notes:</SubText> <Text>{rm.additional}</Text>
							</ItemWrapper>
						)}
					</div>
				)}
				{rr && (
					<div className="flex flex-col max-w-md gap-1.5">
						...
						{rr.additional && (
							<ItemWrapper>
								<SubText>Additional Notes:</SubText> <Text>{rr.additional}</Text>
							</ItemWrapper>
						)}
					</div>
				)}
...
const SubText = ({ children }: { children: React.ReactNode }) => <Text className="text-foreground/50">{children}</Text>;

const ItemWrapper = ({ children }: { children: React.ReactNode }) => <span className="leading-tight">{children}</span>;
			...
			<CarouselContent>
				{images.map((image, index) => (
					<CarouselItem key={`${image.alt}-${index}`}>
						<Image src={image.src} alt={image.alt} width={1920} height={1080} className="rounded-lg" />
					</CarouselItem>
				))}
			</CarouselContent>
					...
					{images.map((image, index) => (
						<span
							...
							className={cn(
								...
								index === current - 1 ? "bg-primary" : "border md:border-2 border-white/25",
							)}
